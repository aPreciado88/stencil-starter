...
export class EmojiRain {
  ...
  @Prop({ mutable: true }) drops: number = 250;
  ...
  componentWillLoad(): void {
    // wait for canvas to load before calling start()
    setTimeout(() => {
      if(this.active){
        this.start();
      }
  ...
  @Watch("runAnimation")
  onActiveChange(): void {
    // keeps animation running when runAnimation changes
    if(this.active){
      this.animate();
    }
  }
  ...
  @Method()
  async makeItRain(): Promise<void> {
    this.start();
  }

  @Method()
  async stopTheRain(): Promise<void> {
    this.stop();
  }
  ...
  private fromCodePoint = (codepoint: string): string => {
  ...
  private giveMeARandomEmoji = (): Emoji => {
    let emoji: Emoji = {
      code: '',
      char: '',
      x: 0,
    ...
    emoji.code = this.emojiUnicode[Math.floor((Math.random() * this.totalEmoji))];
  ...
  private paintEmoji = (emoji: Emoji): void => {
  ...
  private boundAnimate = (): Function => {
    let self = this;
    return function () {
      return self.animate.bind(this);
    }
  }

  private animate = (): void => {
    this.runAnimation = true;
    setTimeout(() => this.runAnimation = false, 1);
    let self = this;
    this.timeout = setTimeout(function() {
      self.animationFrame = window.requestAnimationFrame(self.boundAnimate);
      self.context.clearRect(0, 0, self.canvas.width, self.canvas.height);
      for (let i = 0; i < self.dropsForDrawing.length; i++) {
  ...
  private generateDrops = (): void => {
    ...
    for (let i = 0; i < this.drops; i++) {
      let emoji: Emoji = this.giveMeARandomEmoji();
      emoji.arrayIndex = i;
  ...
  render() {
    return (
      <Host id="emoji-rain">
        <canvas id="canvas" class={!this.runAnimation ? 'hidden' : '' } ref={el => this.canvas = el as HTMLCanvasElement}></canvas>
      </Host>
    );
  }
}
